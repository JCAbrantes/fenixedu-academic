...
import org.fenixedu.academic.domain.InstitutionCodeProvider;
...
public class ThesisRenatesReportFile extends QueueJobWithFile {
    ...
    private Spreadsheet retrieveIndividualThesisData() {
        ...
        Set<CurriculumGroup> collect =
                Bennu.getInstance().getProgramConclusionSet().stream().flatMap(pc -> pc.getCourseGroupSet().stream())
                        .filter(cg -> cg.getDegreeType().isSecondCycle()).flatMap(cg -> cg.getCurriculumModulesSet().stream())
                        .map(cm -> (CurriculumGroup) cm).filter(cm -> cm.isConclusionProcessed()).collect(Collectors.toSet());

        Set<Thesis> thesisSet = FenixFramework.getDomainRoot().getBennu().getThesesSet();
        ...
        for (CurriculumGroup group : collect) {
            ...
            if (thesis == null) {
                Optional<Thesis> thesis2 = thesisSet.stream().filter(t -> t.getStudent().equals(group.getStudent())).findAny();
                if (!thesis2.isPresent()) {
                    continue;
                }
            ...
            InstitutionCodeProvider institutionCodeProvider = FenixEduAcademicConfiguration.getInstitutionCodeProvider();
    ...
    private Enrolment getThesisEnrolment(CurriculumGroup group) {
        Optional<CurriculumLine> curriculumLineOpt = group.getAllCurriculumLines().stream()
                .filter(e -> e.isDismissal() && ((Dismissal) e).getCurricularCourse() != null
                        && ((Dismissal) e).getCurricularCourse().isDissertation()
                        || e.isEnrolment() && ((Enrolment) e).isDissertation())
                .findAny();
        ...
        if (curriculumLine instanceof Dismissal) {
            ...
            if (dismissal.getEnrolments().isEmpty()) {
                return null;
            }
            if (dismissal.getEnrolments().size() > 1) {
            }
            return dismissal.getEnrolments().iterator().next();
        }
