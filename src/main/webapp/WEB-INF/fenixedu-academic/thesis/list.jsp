	...
	<table class="tstyle1 thlight thright mtop025 table">
	    ...
	    <tbody>
	        <c:forEach var="queuejob" items="${doneQueuejobs}">               
		    	<tr>
	        		<td>
						<c:out value="${queuejob.requestDate.toString(\"dd-MM-yyyy hh:mm:ss\")}" />                    
	        		</td>
	        		<td>
	        			<c:out value="${queuejob.jobEndTime.toString(\"dd-MM-yyyy hh:mm:ss\")}"/>
	        		</td>
	        		<td>
	        			<c:out value="${queuejob.person.name}"/>
	        		</td>	        		
	        		<td>	                	            	
						<spring:url var="downloadExcel" value="/thesis-test/download/renates-report/${queuejob.filename}" />
...
<p> 
	<spring:url var="requestReport" value="/thesis-test/requestRenatesReport"/>
